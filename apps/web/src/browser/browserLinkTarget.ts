...
export interface ResolveLinkTargetInput {
  ...
  readonly previewPattern?: string;
...
export function resolveLinkTarget(input: ResolveLinkTargetInput): BrowserLinkTarget {
  ...
  if (
    input.preference !== "app" &&
    !urlMatchesPreviewLinkPattern(input.previewPattern ?? "", input.url)
  )
    return "system";
 ...
 */
export async function resolveBrowserLinkTargetPreference(url?: string): Promise<BrowserLinkTarget> {
  await ensureClientSettingsHydrated();
  const settings = getClientSettings();
  return url && urlMatchesPreviewLinkPattern(settings.openLinksInPreviewPattern, url)
    ? "app"
