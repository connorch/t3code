...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  let userBodyText = resolvedContext.text;
  while (true) {
    const extracted = extractTrailingThreadTranscript(userBodyText);
    if (!extracted.transcript) break;
    threadTranscripts.unshift(extracted.transcript);
    userBodyText = extracted.promptText;
  }
