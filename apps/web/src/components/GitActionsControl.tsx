...
export default function GitActionsControl({
  ...
  const openExistingPr = useCallback(async () => {
    ...
    // Merged PRs stay viewable and reach the shell here: the in-app panel above
    // only opens for `state === "open"`.
    const prUrl =
      openPr?.url ??
      (gitStatusForActions?.pr?.state === "merged" ? gitStatusForActions.pr.url : null);
    if (!prUrl) {
      toastManager.add({
        type: "error",
